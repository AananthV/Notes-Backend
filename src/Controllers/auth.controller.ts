...
class AuthController implements Controller {
    ...
    private initialiseRoutes() {
        ...
        this.router.get(`${this.path}/verify`, validationMiddleware(VerifyTokenDto), this.verifyToken);
        this.router.post(
            `${this.path}/initiateResetPassword`,
            validationMiddleware(ForgotPasswordDto),
            this.initiateResetPassword,
        );
        this.router.post(`${this.path}/resetPassword/:id`, validationMiddleware(ResetPasswordDto), this.resetPassword);
        this.router.post(
    ...
    private login = async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (oldUser == null) {
            ...
            const match = bcrypt.compare(password, oldUser.password);
            if (match) {
                // if passwords match
    ...
    private checkUsernameAvailability = async (req: Request, res: Response, next: NextFunction) => {
    ...
    private register = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            req.body.password = await bcrypt.hash(req.body.password, config.get<number>('crypt.salt_rounds'));
            ...
            if (config.get<boolean>('mail.enabled') == true) {
                if (config.get('mail.sendgrid_key') != '') {
                    const mailerResponse = await sendVerificationMail(req.body.email, newUser.verificationCode);
                } else {
                    next(
                        new HttpException({
                            status: 500,
    ...
    private initiateResetPassword = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const uniq_id = await this.generateRandomString(15);
            ...
            return res.status(200).jsonp({
                status: 200,
                success: true,
                msg: 'An email for resetting password link has been sent to your registered email!',
            });
        } catch (err) {
            return next(
                new HttpException({
    ...
    private resetPassword = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            if (password == confirmPassword) {
                const newPassword = await bcrypt.hash(password, config.get<number>('crypt.salt_rounds'));
                const requestId = req.params.id;
                const requestedUser = await this.user.findOne({
